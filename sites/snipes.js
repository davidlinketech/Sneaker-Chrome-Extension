...
var shoeNameInfo =  html.split('<a class="b-pdp-brand"')[2].split('<span class="h-hide">')[0].split('\n')
var brand = shoeNameInfo[1]
var name = shoeNameInfo[3]
var shoename = brand+" "+name 
var shoeimage = html.split('{"@context":"http://schema.org","@type":"Product","image":["')[1].split('","')[0]

var btnHtml = ""          //html for sizebuttons
var sizes = {}
var sizeDivs = document.getElementsByClassName('b-pdp-attribute b-pdp-attribute--size js-pdp-attributes--size')[0].children
for(k=0; k<sizeDivs.length; k++){
    //sizeinfo
    var size = sizeDivs[k].children[0].getAttribute('data-variant-id')
    var sizename = sizeDivs[k].children[0].getAttribute('data-value')
    
    //every size with its info is beeing stored in the sizes object
    sizes[size] = {}
    if(sizeDivs[k].children[0].children[0].className.includes('sold-out') || sizeDivs[k].children[0].children[0].className.includes('in-store-only')){
        sizes[size]["stock"] = false
    }else{
        sizes[size]["stock"] = true
    }
    sizes[size]["sizename"] = sizename
    
    //gives size button a color depending on the stock of the size
    if(sizes[size]["stock"]){
        btnHtml += '<button id="btn'+k.toString()+'" style="background-color: #2c3f82; height: 25px; width: 50px; text-align: center; cursor: pointer; border-radius: 4px; border: 3px solid #00d92f; color: white; font-size: 15px; margin: 2px;">'+sizename+'</button>'
    }else{
        btnHtml += '<button id="btn'+k.toString()+'" style="background-color: #2c3f82; height: 25px; width: 50px; text-align: center; cursor: pointer; border-radius: 4px; border: 3px solid #e60f0b; color: white; font-size: 15px; margin: 2px;">'+sizename+'</button>'
    }
}
...
document.getElementsByClassName('b-pdp-product-info')[0].appendChild(div);      
...
document.getElementById('startScript').addEventListener('click', mainSnipes)       
document.getElementById('stopScript').addEventListener('click', stopMain)


async function mainSnipes(){
    runScript = true;
    while(runScript){
        try{
